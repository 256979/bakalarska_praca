...
for i in range(len(dicom)):
    ...
    # SimpleITK array: (z, y, x) in LPS space
    native_lps = sitk.GetArrayFromImage(img_sitk).astype(np.float32)  # (z, y, x)

    # Reorder to (x, y, z) for consistency with NIfTI
    native_lps = np.transpose(native_lps, (2, 1, 0))  # (x, y, z)

    # Build affine from SimpleITK metadata (LPS)
    ...
    affine_lps = np.eye(4)
    affine_lps[:3, :3] = direction * spacing  # scale direction cosines
    ...
    # VISUAL CHECK: Native vs P1 vs P2 vs P3



    seg_indices = np.where(seg)
    z_coords = seg_indices[2]
    mid_slice = int(np.median(z_coords))

    native_slice = ct_hu[:, :, mid_slice]
    p1_slice = data_P1[:, :, mid_slice]
    p2_slice = data_P2[:, :, mid_slice]
    p3_slice = data_P3[:, :, mid_slice]
    seg_slice = seg[:, :, mid_slice]

    fig, axes = plt.subplots(1, 4, figsize=(22, 6))

    # --- Native ---
    axes[0].imshow(native_slice, cmap='gray')
    axes[0].imshow(np.ma.masked_where(~seg_slice, seg_slice), cmap='autumn', alpha=0.4)
    axes[0].set_title(f"Native (slice {mid_slice})")
    axes[0].axis('off')

    # --- Phase 1 ---
    axes[1].imshow(p1_slice, cmap='gray')
    axes[1].imshow(np.ma.masked_where(~seg_slice, seg_slice), cmap='autumn', alpha=0.4)
    axes[1].set_title("Phase 1")
    axes[1].axis('off')

    # --- Phase 2 ---
    axes[2].imshow(p2_slice, cmap='gray')
    axes[2].imshow(np.ma.masked_where(~seg_slice, seg_slice), cmap='autumn', alpha=0.4)
    axes[2].set_title("Phase 2")
    axes[2].axis('off')

    # --- Phase 3 ---
    axes[3].imshow(p3_slice, cmap='gray')
    axes[3].imshow(np.ma.masked_where(~seg_slice, seg_slice), cmap='autumn', alpha=0.4)
    axes[3].set_title("Phase 3")
    axes[3].axis('off')
    ...
    print(
        f"Unif  Native:{Uniformity_Native:.6f}  P1:{Uniformity_P1:.6f}  P2:{Uniformity_P2:.6f}  P3:{Uniformity_P3:.6f}")
    ...
    csv_path = "/home/feketeova/Documents/results_nove.csv"
